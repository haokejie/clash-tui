...
use crate::{
    ...
    config::{
        AppSettings, BaseConfig, ProfileCatalog, RuleProviderDownloadProxy, dns,
...
const ROOT_RUNTIME_PATCH_KEYS: &[&str] = &["allow-lan", "ipv6", "log-level", "unified-delay", "tunnels"];
const SUBSCRIPTION_PLACEHOLDER_PROXY_SERVERS: &[&str] = &["10.255.255.255"];
...
impl RuntimeConfig {
    ...
    pub fn patch_config(&mut self, patch: &Mapping) {
        ...
        copy_runtime_root_keys(config, patch);
        merge_tun_overrides(config, patch);
    }
...
fn copy_runtime_root_keys(config: &mut Mapping, patch: &Mapping) {
    ROOT_RUNTIME_PATCH_KEYS
        .iter()
        .filter_map(|key| patch.get(*key).map(|value| ((*key).to_owned(), value.clone())))
        .for_each(|(key, value)| {
            config.insert(key.into(), value);
        });
}

fn merge_tun_overrides(config: &mut Mapping, patch: &Mapping) {
    let Some(Value::Mapping(patch_tun)) = patch.get("tun") else {
        return;
    };
...
impl RuntimeConfigGenerator {
    ...
    pub async fn generate_with_dns_override(&self, enable_dns_settings: Option<bool>) -> Result<RuntimeConfigResult> {
        ...
        apply_rule_provider_download_proxy(
            &mut config,
            app_settings.rule_provider_download_proxy.unwrap_or_default(),
        );
        apply_controller_boundary(&mut config, &self.paths, &app_settings);
        self.apply_proxy_chain_config(&mut config)?;
        sanitize_subscription_placeholder_proxies(&mut config);
    ...
    async fn load_clash(&self) -> Result<BaseConfig> {
    ...
    async fn current_profile_mapping(&self, profiles: &ProfileCatalog) -> Result<(Mapping, bool)> {
    ...
    async fn apply_profile_enhancements(&self, mut config: Mapping, profiles: &ProfileCatalog) -> Result<Mapping> {
    ...
    async fn load_merge_item(&self, profiles: &ProfileCatalog, uid: &str) -> Result<Option<Mapping>> {
    ...
    async fn load_script_item(&self, profiles: &ProfileCatalog, uid: &str) -> Result<Option<String>> {
        let Some(path) = self.profile_item_path(profiles, uid).await? else {
            return Ok(None);
        };
    ...
    async fn profile_item_path(&self, profiles: &ProfileCatalog, uid: &str) -> Result<Option<PathBuf>> {
...
fn sanitize_subscription_placeholder_proxies(config: &mut Mapping) {
    let removed = remove_subscription_placeholder_proxy_entries(config);
    if removed.is_empty() {
        return;
    }

    prune_removed_proxy_group_references(config, &removed);
    prune_removed_dialer_proxy_references(config, &removed);
}

fn remove_subscription_placeholder_proxy_entries(config: &mut Mapping) -> BTreeSet<String> {
    ...
    proxies.retain(|proxy| {
        let should_remove = proxy.as_mapping().is_some_and(is_subscription_placeholder_proxy);
        if should_remove
            && let Some(name) = proxy
                .as_mapping()
...
fn is_subscription_placeholder_proxy(proxy: &Mapping) -> bool {
    proxy
        .get("server")
        .and_then(Value::as_str)
        .map(str::trim)
        .is_some_and(|server| SUBSCRIPTION_PLACEHOLDER_PROXY_SERVERS.contains(&server))
}

fn prune_removed_proxy_group_references(config: &mut Mapping, removed: &BTreeSet<String>) {
    ...
    for group in groups {
        ...
        let Some(Value::Sequence(proxies)) = group.get_mut("proxies") else {
            continue;
        };

        let had_proxies = !proxies.is_empty();
        proxies.retain(|proxy| {
            proxy
                .as_str()
                .map(str::trim)
                .is_none_or(|proxy| !removed.contains(proxy))
        });
        if had_proxies && proxies.is_empty() {
            proxies.push(Value::String("DIRECT".into()));
        }
    }
}

fn prune_removed_dialer_proxy_references(config: &mut Mapping, removed: &BTreeSet<String>) {
    let Some(Value::Sequence(proxies)) = config.get_mut("proxies") else {
        return;
    };
    ...
    for proxy in proxies {
        ...
        if proxy
            ...
            .and_then(Value::as_str)
            .map(str::trim)
            .is_some_and(|dialer_proxy| removed.contains(dialer_proxy))
        {
            proxy.remove("dialer-proxy");
        }
    }
...
fn apply_app_settings_overrides(config: &mut Mapping, app_settings: &AppSettings) {
...
fn apply_rule_provider_download_proxy(config: &mut Mapping, strategy: RuleProviderDownloadProxy) {
    if strategy != RuleProviderDownloadProxy::Direct {
        return;
    }
    let Some(Value::Mapping(providers)) = config.get_mut("rule-providers") else {
        return;
    };
    ...
    for provider in providers.values_mut() {
        ...
        let is_http = provider
            .get("type")
            .and_then(Value::as_str)
            .is_some_and(|provider_type| provider_type.eq_ignore_ascii_case("http"));
        if is_http {
...
fn apply_controller_boundary(config: &mut Mapping, paths: &AppPaths, app_settings: &AppSettings) {
    #[cfg(unix)]
    {
...
fn external_controller_port(app_settings: &AppSettings) -> u16 {
    app_settings
...
mod tests {
    ...
    use super::{RuntimeConfig, RuntimeConfigGenerator};
    use crate::{
        AppPaths,
        config::{AppSettings, ConfigStore, RuleProviderDownloadProxy},
    };
    ...
    async fn runtime_generator_writes_opt_in_external_controller() {
        ...
        loaded.app_settings = AppSettings {
    ...
    async fn runtime_generator_strips_profile_external_controller_when_disabled() {
        ...
        loaded.profiles.items = Some(vec![crate::config::ProfileEntry {
            uid: Some("L001".into()),
            itype: Some("local".into()),
            file: Some("demo.yaml".into()),
            name: Some("Demo".into()),
            ..crate::config::ProfileEntry::default()
        }]);
        loaded
            .profiles
    ...
    #[tokio::test]
    async fn runtime_generator_inherits_rule_provider_download_proxy_by_default() {
        let root = temp_root("rule-provider-proxy-inherit");
        let _ = fs::remove_dir_all(&root);

        let store = ConfigStore::new(AppPaths::from_home(&root));
        let mut loaded = store.initialize().await.expect("initialize configs");
        fs::write(
            root.join("profiles").join("rules.yaml"),
            r"rule-providers:
  Direct:
    type: http
    behavior: domain
...
",
        ...
        .expect("write profile");
        loaded.profiles.current = Some("Rrules".into());
        loaded.profiles.items = Some(vec![crate::config::ProfileEntry {
            uid: Some("Rrules".into()),
        ...
        assert_eq!(runtime_rule_provider_proxy(&runtime, "Direct"), None);
        assert_eq!(runtime_rule_provider_proxy(&runtime, "ExistingProxy"), Some("Proxy"));
        assert_eq!(runtime_rule_provider_proxy(&runtime, "LocalFile"), None);

        let _ = fs::remove_dir_all(&root);
    }

    #[tokio::test]
    async fn runtime_generator_forces_http_rule_provider_download_proxy_to_direct() {
        let root = temp_root("rule-provider-proxy-direct");
        let _ = fs::remove_dir_all(&root);
...
",
        ...
        assert_eq!(runtime_rule_provider_proxy(&runtime, "Direct"), Some("DIRECT"));
        assert_eq!(runtime_rule_provider_proxy(&runtime, "ExistingProxy"), Some("DIRECT"));
        assert_eq!(runtime_rule_provider_proxy(&runtime, "LocalFile"), None);
...
",
    ...
    #[tokio::test]
    async fn runtime_generator_filters_subscription_placeholder_proxies() {
        let root = temp_root("subscription-placeholder-proxies");
        let _ = fs::remove_dir_all(&root);

        let store = ConfigStore::new(AppPaths::from_home(&root));
        let mut loaded = store.initialize().await.expect("initialize configs");
        fs::write(
            root.join("profiles").join("bigme.yaml"),
            r"proxies:
  - name: 剩余流量：234.78 GB
    type: trojan
...
",
        ...
        .expect("write profile");
        loaded.profiles.current = Some("Rbigme".into());
        loaded.profiles.items = Some(vec![crate::config::ProfileEntry {
            uid: Some("Rbigme".into()),
            itype: Some("remote".into()),
            file: Some("bigme.yaml".into()),
            name: Some("BigME.Pro".into()),
            ..crate::config::ProfileEntry::default()
        }]);
        loaded
            .profiles
        ...
        let generator = RuntimeConfigGenerator::from_loaded(&loaded);
        generator.generate().await.expect("generate runtime");
        let runtime_yaml = tokio::fs::read_to_string(root.join("mihomo-runtime.yaml"))
            .await
            .expect("read runtime yaml");
        let runtime = crate::yaml::read_mapping(root.join("mihomo-runtime.yaml"))
            .await
            .expect("read runtime");

        assert!(!runtime_yaml.contains("10.255.255.255"));
        assert!(!runtime_yaml.contains("剩余流量：234.78 GB"));
        assert!(!runtime_yaml.contains("套餐到期：长期有效"));

        let proxy_names = runtime
            .get("proxies")
            .and_then(Value::as_sequence)
            .expect("runtime proxies")
            .iter()
            .filter_map(|proxy| {
                proxy
                    .as_mapping()
            ...
            .collect::<Vec<_>>();
        assert_eq!(proxy_names, vec!["HK-1", "US-1"]);

        let proxies_group = runtime_group_proxies(&runtime, "Proxies");
        assert_eq!(proxies_group, vec!["HK-1", "US-1"]);
        let info_only_group = runtime_group_proxies(&runtime, "Info Only");
        assert_eq!(info_only_group, vec!["DIRECT"]);
...
"#;
    ...
    fn runtime_group_proxies(runtime: &Mapping, group_name: &str) -> Vec<String> {
        runtime
            .get("proxy-groups")
            .and_then(Value::as_sequence)
            .into_iter()
    ...
    fn runtime_rule_provider_proxy<'a>(runtime: &'a Mapping, provider_name: &str) -> Option<&'a str> {
        runtime
            .get("rule-providers")
            .and_then(Value::as_mapping)
